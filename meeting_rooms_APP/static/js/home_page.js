    ...
    $(document).on("click","#totalTable tbody tr td button.btn", function() { // any button
        ...
        for(i = 0; i < all_rooms_data.length; ++i) {
             if (all_rooms_data[i]['user_id'] == session_data['userId'] ){
                    alert('user can reserve a meeting room already')
...
//     data_view.find('tr').remove();
        ...
        for(i = 0; i < data.length; ++i) {
             ...
             if( data[i]['booking_status'] == 2){
                var btnBooking = '<button type="button"   data-toggle="modal" data-target="#exampleModal" class="btn btn-primary btn-sm"  row_id ="'+data[i]['id']+'" disabled>Already Booking</button>'
             }
             else{
